...
import timeit
import warnings
import multitaper.mtspec as spectrogram
# Helper Functions #
n_cpus = os.cpu_count()//2


def choose_wave_spec_multitaper(brainwave, tepdata, Fs, psdclass):
    def multitaper_parallel(tepdata, Fs, freq_range):
        try:
            tt, f, Hss = spectrogram.spectrogram(tepdata, 1 / Fs, twin=window_parameter[0],
                                                    olap=window_parameter[1],
                                                    nw=time_bandwidth,
                                                    kspec=num_tapers,
                                                    fmin=freq_range[0],
                                                    fmax=freq_range[1],
            ...
            Sxx = np.zeros((1,1)) + 1e-10
            tt = np.zeros(1)
            f = np.zeros(1)
            print(e)
        return Hss, tt, f

    input_length = len(tepdata)/Fs

    window_size = 0.3
    num_step = 100
    window_step = (input_length - window_size) / (num_step - 1)
    overlapping = 1 - window_step / window_size

    window_parameter = [window_size, overlapping]  # (100 steps total)
    time_bandwidth = 3.5  # Set time-bandwidth product
    num_tapers = 5  # Set number of tapers (optimal is time_bandwidth*2 - 1)

    ParallelOUT = Parallel(n_jobs=n_cpus)(delayed(multitaper_parallel)(tepdata[:, i], Fs, [8, 150])
                                              for i in range(len(tepdata.T)))


    # Rearrange the Sxx so that whenever the size of data is 1*1, it will be substituted with [1e-10]*f*t
    # Find the size of the data with the maximum size
    lenf,lent = 143, num_step

    for i in range(len(ParallelOUT)):
        if ParallelOUT[i][0].shape == (1, 1):
            ParallelOUT[i] = list(ParallelOUT[i])
            ParallelOUT[i][0] = np.ones((lenf, lent))*1e-10
        else:
            tt = ParallelOUT[i][1]
            f = ParallelOUT[i][2][:,0]
    try:
        Sxx = np.array([ParallelOUT[i][0] for i in range(len(ParallelOUT))])
        Sxx_dB = 10 * np.log10(Sxx)
    except:
        print('Error in calculating the Sxx_dB')

    for j in brainwave:
        if j == 'alpha':  # select alpha brainwave
            f_alpha_ind = np.where((f >= 8) & (f <= 12))
            Sxx_alpha = Sxx_dB[:, f_alpha_ind[0], :]
            f_alpha = f[f_alpha_ind[0]]
            psdclass.addalphaspectrogram(Sxx_alpha)
            # Stack the frequency
            psdclass.ft = np.hstack((psdclass.ft, f_alpha))
            psdclass.t = tt
        if j == 'beta':  # select beta brainwave
            f_beta_ind = np.where((f >= 12) & (f <= 30))
            Sxx_beta = Sxx_dB[:, f_beta_ind[0], :]
            f_beta = f[f_beta_ind[0]]
            psdclass.addbetaspectrogram(Sxx_beta)
            # Stack the frequency
            psdclass.ft = np.hstack((psdclass.ft, f_beta))
            psdclass.t = tt
        elif j == 'gamma':  # select gamma brainwave
            f_gamma_ind = np.where((f >= 30) & (f <= 70))
            Sxx_gamma = Sxx_dB[:, f_gamma_ind[0], :]
            f_gamma = f[f_gamma_ind[0]]
            psdclass.addgammaspectrogram(Sxx_gamma)
            # Stack the frequency
            psdclass.ft = np.hstack((psdclass.ft, f_gamma))
            psdclass.t = tt
        elif j == 'high_gamma':  # select high gamma brainwave
            f_high_gamma_ind = np.where((f >= 70) & (f <= 150))
            Sxx_high_gamma = Sxx_dB[:, f_high_gamma_ind[0], :]
            f_high_gamma = f[f_high_gamma_ind[0]]
            psdclass.addhgammaspectrogram(Sxx_high_gamma)
            # Stack the frequency
            psdclass.ft = np.hstack((psdclass.ft, f_high_gamma))
            psdclass.t = tt
    return psdclass

if __name__ == '__main__':
    # ############################## Test for Spectrogram ################################
    def multitaper_parallel(tepdata, Fs, freq_range, **kwargs):
        window_parameter = kwargs.get('window_parameter', [1, 0.99])  # 1 seconds with 99% overlap (1ms step size)
        time_bandwidth = kwargs.get('time_bandwidth', 3.5)  # Set time-bandwidth product
        num_tapers = kwargs.get('num_tapers', 5)  # Set number of tapers (optimal is time_bandwidth*2 - 1)
        # try:
        tt, f, Sxx, HSS = spectrogram.spectrogram(tepdata, 1 / Fs, twin=window_parameter[0],
                                                olap=window_parameter[1],
                                                nw=time_bandwidth,
                                                kspec=num_tapers,
                                                fmin=freq_range[0],
    ...
    # n_channels = 4  # Number of trials
    Fs = 2000  # Sampling frequency
    # duration = 1  # Duration of the signal
    # # Generate time series data
    # np.random.seed(0)
    # t = np.linspace(0, duration, int(Fs*duration), endpoint=False)
    # x = np.zeros((n_channels, len(t)))
    # for i in range(n_channels):
    #     # Generate random data
    #     x[i, :] = np.random.randn(len(t)) + np.sin(2 * np.pi * 50 * t) + np.sin(2 * np.pi * 100 * t)
    # x = x.T
    x = np.load('x.npy')
    # window_parameter = [0.5, 0.99]  # 1 seconds with 99% overlap (1ms step size)
    time_bandwidth = 3.5  # Set time-bandwidth product
    num_tapers = 5  # Set number of tapers (optimal is time_bandwidth*2 - 1)
    ...
    # Sxx, tt, f = multitaper_parallel(x[:, 0], Fs, [8, 150],
    #                                   window_parameter=window_parameter,
    #                                   time_bandwidth=time_bandwidth,
    #                                   num_tapers=num_tapers)

    Data = [multitaper_parallel(x[:, i], Fs, [8, 150],
                                    window_parameter=window_parameter,
                                    time_bandwidth=time_bandwidth,
                                    num_tapers=num_tapers) for i in range(x.shape[1])]

    Sxx = np.array([Data[i][0] for i in range(len(Data))])
    tt = Data[0][1]
    f = Data[0][2][:,0]
    ...
    fig, ax = plt.subplots()
    im = ax.imshow(Sxx_dB, extent=[tt[0], tt[-1], f[-1,0], f[0,0]], aspect='auto')
    fig.colorbar(im, ax=ax, label='PSD (dB)', shrink=0.8)
    ax.set_xlabel("Time (s)")
    ax.set_ylabel("Frequency (Hz)")
    im.set_cmap(plt.cm.get_cmap('rainbow'))
    ax.invert_yaxis()
    plt.show()
...
#         n_jobs = max(n_cpus - 1, 1) if n_jobs is None else n_jobs
